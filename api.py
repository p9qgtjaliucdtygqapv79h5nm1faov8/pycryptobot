...
class AuthAPI(AuthAPIBase):
    ...
    def marketMin(self, market, amount, price) -> [float, float]:
        product = self.authAPI('GET', f'products/{market}')

        if 'base_min_size' not in product or 'min_market_funds' not in product:
            return [amount, price]

        base_min_size = str(product['base_min_size'].values[0])
        min_market_funds = str(product['min_market_funds'].values[0])
        return [base_min_size, min_market_funds]

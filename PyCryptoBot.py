...
class PyCryptoBot():
    ...
    def checkBalance(self, account, last_action=''):
        api = CBAuthAPI(self.getAPIKey(), self.getAPISecret(), self.getAPIPassphrase(), self.getAPIURL())
        base_min_size, min_market_funds = api.marketMin(self.getMarket(), 0.001, 50)
        if last_action == 'SELL' and account.getBalance(self.getQuoteCurrency()) < min_market_funds:
        ...
        elif last_action == 'BUY' and account.getBalance(self.getBaseCurrency()) < base_min_size:
            raise Exception('Insufficient available funds to place sell order: ' + str(account.getBalance(
                self.getBaseCurrency())) + ' < ' + base_min_size + self.getBaseCurrency() + "\nNote: A manual limit order places a hold on available funds.")
    ...
    def startApp(self, account, last_action='', banner=True):
        ...
        if self.isLive() == 1:
            if self.getExchange() == 'binance':
                ...
                self.checkBalance(account, last_action)
